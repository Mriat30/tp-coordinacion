...
**GIL de Python y por qué aquí casi no limita.** El intérprete de CPython usa el *Global Interpreter Lock*: en general, solo un hilo ejecuta bytecode de Python a la vez. Eso penaliza el paralelismo cuando el trabajo es **CPU bound** (mucho cómputo puro en Python sin liberar el GIL): varios hilos no aceleran porque compiten por el mismo candado de ejecución. En Sum el trabajo de los dos hilos es en cambio mayormente **I/O bound**: esperan en el cliente de RabbitMQ (lectura de colas y exchanges, escritura de mensajes). Mientras un hilo bloquea en red, el intérprete puede ceder la ejecución al otro; el tiempo de CPU dentro del `Lock` (actualizar un diccionario, sumar `FruitItem`, serializar) es corto frente al tiempo de espera del broker. Por eso usar `threading` para separar datos y control es razonable en este diseño: el cuello de botella suele ser la latencia y el throughput de mensajes, no el paralelismo de CPU. Si la suma implicara, por ejemplo, procesamiento numérico masivo en Python por mensaje, convendría valorizar procesos (`multiprocessing`) u otra estrategia que evite el GIL para esa parte.
